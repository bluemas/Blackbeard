...
TTcpListenPort *TCPSocket::OpenTcpListenPort(short localport) {
...
#endif
  // create a socket
  if ((TcpListenPort->ListenFd = socket(AF_INET, SOCK_STREAM, 0)) ==
      BAD_SOCKET_FD) {
  ...
  if (setsockopt(TcpListenPort->ListenFd, SOL_SOCKET, SO_REUSEADDR,
  ...
  if (bind(TcpListenPort->ListenFd, (struct sockaddr *) &myaddr,
           sizeof(myaddr)) < 0) {
    CloseTcpListenPort(&TcpListenPort);
    perror("bind failed");
    return (NULL);
...
TTcpConnectedPort *TCPSocket::OpenTcpConnection(const char *remotehostname,
  ...
  if ((TcpConnectedPort->ConnectedFd = socket(AF_INET, SOCK_STREAM, 0)) ==
      BAD_SOCKET_FD) {
    CloseTcpConnectedPort(&TcpConnectedPort);
    freeaddrinfo(result);
    perror("socket failed");
  ...
  if (connect(TcpConnectedPort->ConnectedFd, result->ai_addr,
              result->ai_addrlen) < 0) {
    CloseTcpConnectedPort(&TcpConnectedPort);
    freeaddrinfo(result);
