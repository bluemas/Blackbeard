...
float LineRecognizer::calculateLineOffset(Mat& orgImg, Mat& synthImg, bool &crossDetect) {
  ...
  if (fabs(minMaxCx)==DBL_MAX) {
    printf("Line is not detected\n");
    minMaxCx = roi.cols/2;
  }
...
#if 0
  if (crossDetect == true || (abs(mPrevOffset - offsetfromcenter) > 0.2 && mIsReset == false)) {
    printf("offset is not accurate. set offset to zero\n");
    offsetfromcenter = 0.0;
  } else {
    mPrevOffset = offsetfromcenter;
    mIsReset = false;
  }
#else
  if (crossDetect == true) {
